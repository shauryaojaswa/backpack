  ...
  items.forEach(item => {
    html += `
      <div class="menu-item">
        ...
        <button class="add-to-cart" data-id="${item.id}" data-name="${escapeHtml(item.name)}" data-price="${item.price}">+</button>
      </div>
    `;
  });
...
function updateCartUI() {
  ...
  cart.forEach(item => {
    ...
    li.appendChild(document.createTextNode(`${item.name} x${item.qty} `));
...
$('#place-order').addEventListener('click', async () => {
  ...
  currentOrderId = remoteId || fallbackId;
  ...
  if (!remoteId) {
    // Persist locally so the admin dashboard (local fallback) can see this order.
...
function listenToOrder() {
  ...
  if (localOrder) {
    ...
    // Also try to sync status in real time from Firestore if this order exists there.
    if (!currentOrderId.startsWith('local-')) {
      unsubscribe = onSnapshot(doc(db, 'orders', currentOrderId), (docSnap) => {
        if (docSnap.exists()) updateStatusUI(docSnap.data().status);
      }, (error) => {
        console.warn('Realtime order listener failed:', error);
      });
    }
    return;
  }
...
function startAutoStatusFlow() {
  // Demo-only flow used when Firestore is unreachable (local fallback orders).
  const stages = ['placed', 'accepted', 'preparing', 'ready'];
  let index = 0;
